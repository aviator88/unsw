...
from eth_async.utils.utils import api_key_required
from eth_async.data.models import TokenAmount, CommonValues, TxArgs
from eth_async.classes import AutoRepr
from eth_async import exceptions
from eth_async.data import types
...
class Transactions:
    ...
    async def find_txs(
        ...
        txs = {}
        coin_txs = (await self.client.network.api.functions.account.txlist_all(address))
        for tx in coin_txs:
            if (
    ...
    async def simple_tx(
        ...
        failed_text = 'Failed to transact'
        
        from_address = client.account.address
        to_address = Web3.to_checksum_address(to_address)

        value = amount
        balance = await client.wallet.balance()
        logger.info(f'Client {client.account.address} is OK. Balance: {balance.Ether} {token_symbol}')

        if int(str(balance)) == 0:
            return str(f'{failed_text}: Zero balance')
        ...
        tx_params = {
            ...
            'from': from_address,
            'to': to_address,
            'value': int(str(value.Wei)),
        }

        gas = await client.transactions.estimate_gas(tx_params)
        value = TokenAmount(int(str(value)) - multiplier * int(str(gas)) * int(str(gas_price)), wei = True)

        if int(str(value)) <= 0:
            return str(f'{failed_text}: Low balance')
        ...
        try:
            ...
            return 'Transaction is not in the chain after 600 seconds'
        return receipt
